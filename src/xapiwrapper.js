...
function isDate(date) {
    ...
    if (Object.prototype.toString.call(d) === "[object Date]") {
        // it is a date
        if (isNaN(d.valueOf())) {
            ADL.this.log("Invalid date String passed");
            return null;
        } else {
            return d;
        }
        ...
        // not a date
        ADL.this.log("Invalid date object");
        return null;
    }
}
...
var Config = function () {
    ...
    try {
        ...
        console.error("Exception in Config trying to encode auth: " + e);
    }
...
var XAPIWrapper = function (config, verifyxapiversion) {
    ...
    if (typeof window !== "undefined") this.base = getbase(this.lrs.endpoint);
    else this.base = "http://test";
    ...
    function getbase(url) {
        ...
        if (l.protocol && l.host)
            ...
            this.log("Couldn't create base url from endpoint: " + this.lrs.endpoint);
    }
    ...
    if (verifyxapiversion && testConfig.call(this)) {
        this.XHR_request(this.lrs, this.lrs.endpoint + "about", "GET", null, null,
            function (r) {
                if (r.status == 200) {
                    try {
                        ...
                        if (!versionOK) {
                            ADL.this.log("The lrs version [" + lrsabout.version + "]" +
                                " does not match this wrapper's XAPI version [" + ADL.XAPIWrapper.xapiVersion + "]");
                        }
                    }
                    catch (e) {
                        ADL.this.log("The response was not an about object")
                    }
                }
                else {
                    ADL.this.log("The request to get information about the LRS failed: " + r);
                }
    ...
    this.hash = function (tohash) {
        ...
        try {
            ...
            ADL.this.log("Error trying to hash -- " + e);
            return null;
        }
    };
...
// This wrapper is based on the Experience API Spec version:
     ...
     */
    this.prepareStatement = function (stmt) {
...
// tests the configuration of the lrs object
     ...
     * ADL.this.log("[" + resp_obj.id + "]: " + resp_obj.xhr.status + " - " + resp_obj.xhr.statusText);
     ...
     *     ADL.this.log("[" + obj.id + "]: " + resp.status + " - " + resp.statusText);});
     * >> [4edfe763-8b84-41f1-a355-78b7601a6fe8]: 204 - NO CONTENT
     */
    this.sendStatement = function (stmt, callback) {
        if (this.testConfig()) {
     ...
     */
    this.sendStatements = function (stmtArray, callback) {
        if (this.testConfig()) {
            for (var i in stmtArray) {
                this.prepareStatement(stmtArray[i]);
            }
            var resp = this.XHR_request(this.lrs, this.lrs.endpoint + "statements",
     ...
     * if (ret)
     *     ADL.this.log(ret.statements);
     *
     * >> <Array of statements>
     */
    this.getStatements = function (searchparams, more, callback) {
        if (this.testConfig()) {
            ...
            var res = this.XHR_request(this.lrs, url, "GET", null, this.lrs.auth, callback, null, false, null, this.withCredentials);
     ...
     * >> <Activity object>
     */
    this.getActivities = function (activityid, callback) {
        if (this.testConfig()) {
            var url = this.lrs.endpoint + "activities?activityId=<activityid>";
            url = url.replace('<activityid>', encodeURIComponent(activityid));

            var result = this.XHR_request(this.lrs, url, "GET", null, this.lrs.auth, callback, null, true, null, this.withCredentials);
     ...
     */
    this.sendState = function (activityid, agent, stateid, registration, stateval, matchHash, noneMatchHash, callback) {
        if (this.testConfig()) {
            ...
            this.XHR_request(this.lrs, url, method, stateval, this.lrs.auth, callback, null, null, headers, this.withCredentials);
        }
    };
     ...
     * >> {info: "the state info"}
     */
    this.getState = function (activityid, agent, stateid, registration, since, callback) {
        if (this.testConfig()) {
            ...
            var result = this.XHR_request(this.lrs, url, "GET", null, this.lrs.auth, callback, null, true, this.withCredentials);
     ...
     * >> 404
     */
    this.deleteState = function (activityid, agent, stateid, registration, matchHash, noneMatchHash, callback) {
        if (this.testConfig()) {
            var url = this.lrs.endpoint + "activities/state?activityId=<activity ID>&agent=<agent>&stateId=<stateid>";
            ...
            var result = this.XHR_request(this.lrs, url, "DELETE", null, this.lrs.auth, callback, null, headers, this.withCredentials);
     ...
     */
    this.sendActivityProfile = function (activityid, profileid, profileval, matchHash, noneMatchHash, callback) {
        if (this.testConfig()) {
            ...
            this.XHR_request(this.lrs, url, method, profileval, this.lrs.auth, callback, null, false, headers, this.withCredentials);
        }
    };
     ...
     *                                    "actprofile", null,
     *                                    function(r){ADL.this.log(JSON.parse(r.response));});
     * >> {info: "the profile"}
     */
    this.getActivityProfile = function (activityid, profileid, since, callback) {
        if (this.testConfig()) {
     ...
     * >> XMLHttpRequest {statusText: "NO CONTENT", status: 204, response: "", responseType: "", responseXML: null…}
     */
    this.deleteActivityProfile = function (activityid, profileid, matchHash, noneMatchHash, callback) {
        if (this.testConfig()) {
            var url = this.lrs.endpoint + "activities/profile?activityId=<activity ID>&profileId=<profileid>";
            ...
            var result = this.XHR_request(this.lrs, url, "DELETE", null, this.lrs.auth, callback, null, false, headers, this.withCredentials);
     ...
     * >> <Person object>
     */
    this.getAgents = function (agent, callback) {
        if (this.testConfig()) {
     ...
     */
    this.sendAgentProfile = function (agent, profileid, profileval, matchHash, noneMatchHash, callback) {
        if (this.testConfig()) {
     ...
     */
    this.getAgentProfile = function (agent, profileid, since, callback) {
        if (this.testConfig()) {
     ...
     * >> XMLHttpRequest {statusText: "NO CONTENT", status: 204, response: "", responseType: "", responseXML: null…}
     */
    this.deleteAgentProfile = function (agent, profileid, matchHash, noneMatchHash, callback) {
        if (this.testConfig()) {
            var url = this.lrs.endpoint + "agents/profile?agent=<agent>&profileId=<profileid>";
...
// a config file and from the url query string
    function getLRSObject(config) {
        ...
        if (typeof window !== "undefined") qsVars = parseQueryString();
...
// parses the params in the url query string
     ...
     */
    this.dateFromISOString = function (isostr) {
...
// Synchronous if callback is not provided (not recommended)
     ...
     * @return {object} xhr response object
     */
    this.XHR_request = function (lrs, url, method, data, auth, callback, callbackargs, ignore404, extraHeaders, withCredentials) {
        ...
        function requestComplete() {
            if (!finished) {
                ...
                if (xhr.status === undefined || (xhr.status >= 200 && xhr.status < 400) || notFoundOk) {
                    ...
                    console.log(warning);
                    self.xhrRequestOnError(xhr, method, url, callback, callbackargs);
                    result = xhr;
                    return xhr;
                }
            } else {
     ...
     * };
     */
    this.xhrRequestOnError = function (xhr, method, url, callback, callbackargs) {
    };
};
